...
void appearance(int[HEIGHT][WIDTH], HANDLE h) {
	for (int y = 0; y < HEIGHT; y++)
	{
		for (int x = 0; x < WIDTH; x++)
		{
...
void game(HANDLE h, int map[HEIGHT][WIDTH], COORD hero) {
	while (true) {
...
void moveHero(HANDLE h, COORD& hero, int map[HEIGHT][WIDTH]) {
	while (true) {
		int code = _getch(); // get character
		if (code == 224) {
			code = _getch();
...
void audit(COORD hero, int map[HEIGHT][WIDTH], int& coins_collected, HANDLE h){
